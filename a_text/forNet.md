...
- 代理转发
- iframe无法解决（A页面中嵌入iframe，iframe页面发送请求时，发送站点还是iframe src 对应的域名，而不是A）

## 2. CDN
内容分发网络，根据用户位置分配最近的资源  
用户访问域名 ——> DNS解析 ——> 发现CNAME（别名记录）（假设是CDN负载均衡系统） ——> 智能调度，找到最合适的边缘节点，返回该IP ——> 访问该IP
### 2.1 CDN回溯
即CDN某个节点没有缓存某个资源时，向源站回源获取资源的过程 
### 2.2 CDN分级缓存
分级缓存：边缘节点 ——> 区域/中央节点 ——> 源站  
目标：减轻源站压力；提高热门资源利用率 
### 2.3 CDN如何分发资源的
- 按需缓存：用户请求资源 ——> 边缘节点没有 ——> 向源站请求 ——> 存储在边缘节点上 ——> 返回给用户
- 主动分发：提前将资源推送到指定的节点上（如大型活动资源
...
## 14. defer 与 load、DomContentLoaded 的顺序
![这是图片](./images/defer.png "defer")
